# SKYFORGE — Load Order MO2 panneau gauche — Étape 450

## Objet du fichier

Snapshot complet du **panneau gauche MO2** après les validations et réorganisations documentées jusqu'à l'**étape 450**.

Ce fichier remplace `docs/configuration/SKYFORGE_Load_Order_MO2_panneau_gauche_etape_434.md` comme **référence pratique courante anti-doublon**.

Le snapshot étape 434 reste conservé comme jalon historique.

## Règles de lecture

- Les mods dont le nom se termine par `- FR` sont des traductions personnelles de Fabien et restent **décochés pour le moment**, sauf indication contraire explicite.
- Certains séparateurs sont volontairement vides : cela indique simplement qu'aucun mod n'a encore été installé dans ces blocs.
...
- Ce fichier documente le **panneau gauche MO2**, pas le compteur exact des plugins actifs du panneau droit.
- Le compteur ESP + ESM non-light final post-450 reste donc à confirmer par Fabien via le panneau droit MO2.

## État associé

- Dernière étape documentée : **Étape 450**
- Bloc clôturé provisoirement : **10 - GAMEPLAY COMBAT MAGIC PERKS**
- Prochain bloc prévu : **10.1 - RACES WEREBEASTS VAMPIRES**
- LOOT : non lancé
- DynDOLOD / LOD : non générés
- BodySlide : non généré
- Pandora : déjà généré, Output actif ; ne pas relancer sauf demande explicite ou besoin technique
- Mods `- FR` : décochés sauf indication contraire explicite
